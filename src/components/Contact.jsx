...
const socialLinks = [
  {
    href: 'https://github.com/EmaShalom',
    alt: 'GitHub',
    icon: (
  ...
  {
    href: 'https://www.linkedin.com/in/emmanuel-shalom-208ab12b/',
    alt: 'LinkedIn',
    icon: (
      <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  {
    href: 'https://www.instagram.com/molash.leunamme/',
    alt: 'Instagram',
    icon: (
...
const inputClass =
  'w-full bg-zinc-800 border border-zinc-700 rounded-xl px-4 py-2.5 text-zinc-100 placeholder:text-zinc-500 focus:outline-none focus:border-sky-400 transition-colors';
    ...
    <section id="contact" className="section">
      <div className="max-w-screen-2xl mx-auto px-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-start">
          ...
          <div>
            <h2 className="headline-2 mb-4">{t.contact.title}</h2>
            <p className="text-zinc-400 mb-8">{t.contact.subtitle}</p>
            <div className="flex gap-3">
              {socialLinks.map(({ href, icon, alt }) => (
                <a key={alt} href={href} target="_blank" rel="noopener noreferrer"
                  className="w-12 h-12 border border-zinc-700 rounded-xl flex items-center justify-center text-zinc-400 hover:border-sky-400 hover:text-sky-400 transition-colors">
                  {icon}
                </a>
              ))}
            </div>
          ...
          <form ref={formRef} onSubmit={handleSubmit} className="space-y-5">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm text-zinc-400 mb-1.5">{t.contact.name}</label>
                <input type="text" name="from_name" placeholder={t.contact.namePlaceholder} required className={inputClass} />
              </div>
              <div>
                <label className="block text-sm text-zinc-400 mb-1.5">{t.contact.email}</label>
                <input type="email" name="from_email" placeholder={t.contact.emailPlaceholder} required className={inputClass} />
              </div>
            </div>
            <div>
              <label className="block text-sm text-zinc-400 mb-1.5">{t.contact.message}</label>
              <textarea name="message" placeholder={t.contact.messagePlaceholder} rows={5} required className={`${inputClass} resize-none`} />
            </div>
            <button type="submit" disabled={loading}
              className="w-full bg-sky-400 hover:bg-sky-300 disabled:opacity-60 disabled:cursor-not-allowed text-zinc-900 font-semibold py-3 rounded-xl transition-colors flex items-center justify-center gap-2">
              {loading ? (
                <><span className="w-4 h-4 border-2 border-zinc-900/30 border-t-zinc-900 rounded-full animate-spin"></span>{t.contact.sending}</>
              ) : t.contact.submit}
            </button>
            {status === 'success' && <p className="text-emerald-400 text-sm text-center">{t.contact.success}</p>}
            {status === 'error' && <p className="text-red-400 text-sm text-center">{t.contact.error}</p>}
          </form>
